...
#	}
  ...
  ##estimate sigma^2 to be given to the BART model
    ...
    ## TONY
    .jcall(java_bart_machine, "V", "initAlphaUp", as.integer(length(cov_prior_vec)))
    .jcall(java_bart_machine, "V", "setAlpha0", as.double(alpha_0))
    .jcall(java_bart_machine, "V", "initGibbsSamplesCovSplit", as.integer(num_gibbs), as.integer(length(cov_prior_vec)))
    .jcall(java_bart_machine, "V", "setARD", as.integer(do_ard))
    .jcall(java_bart_machine, "V", "setDoPrior", as.integer(do_prior))
	}
