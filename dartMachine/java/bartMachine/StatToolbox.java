...
public class StatToolbox {
    ...
    // TONY : Random -> Randoms, so I can use mallet samplers
    ...
    // TONY
    public static double sample_from_gamma(double alpha, double beta) {
        return beta * R.nextGamma(alpha);
    }

    // TONY
    public static double[] sample_from_dirichlet(double[] alpha) {
        double[] out = new double[alpha.length];
        double sum_out = 0.0;

        for(int i = 0; i < alpha.length; i++) {
            out[i] = sample_from_gamma(alpha[i], 1.0);
            sum_out += out[i];
        }
