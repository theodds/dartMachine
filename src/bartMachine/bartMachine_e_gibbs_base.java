...
public abstract class bartMachine_e_gibbs_base extends bartMachine_d_init implements Serializable{
	...
	// TONY ADDITIONS: Stuff for updating alpha_0 and monitoring alpha_0 with chain

	protected double alpha_0 = 1.0;
	protected int do_ard = 0;
    	protected double[] log_cov_split_prior;	
	...
	public void initAlphaUp(int p) {
    	...
    	public double[][] gibbs_samples_cov_split_prior;
    ...
    // if(gibbs_sample_num % 100 == 0) System.gc();
	}
...
//		DebugSample(gibbs_sample_num, tree_array_illustration);
    ...
    // TONY: Added so that the update actually occurs
    updateCovSplit(gibbs_sample_num, gibbs_samples_of_bart_trees[gibbs_sample_num]);
	}

    // TONY: Added an abstract method for updating Cov split; implemented in . 
    protected abstract void updateCovSplit(int sample_num, bartMachineTreeNode[] bart_trees); 
